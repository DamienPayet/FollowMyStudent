...
@section('content')
    ...
    <form style='margin-left:10px;' method="POST" action="{{route('users.update', $user->id)}}"
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="nom" name="nom" placeholder="Entrer le nom"
                   value="@if ($user->statut == "eleve"){{ $user->eleve->nom }}@elseif ($user->statut == "admin"){{ $user->admin->nom }}@endif">
        </div>
        <div class="form-group">
            <label for="nom">Prénom *</label>
            <input type="text" class="form-control" id="prenom" name="prenom" placeholder="Entrer le prénom"
                   value="@if ($user->statut == "eleve"){{ $user->eleve->prenom }}@elseif ($user->statut == "admin"){{ $user->admin->prenom }}@endif">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <img src="{{url($user->image_profil)}}" id="img_preview" class="img-rounded"
                 style="height : 150px; border: solid 1px " >
        </div>
        <div class="form-group">
            ...
            <div class="custom-file">
                ...
                <label class="custom-file-label" id="filename"
                       for="validatedCustomFile">{{substr($user->image_profil , 32)}}</label>
            </div>
        </div>
